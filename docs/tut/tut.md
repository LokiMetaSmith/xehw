...
XEH is concatenative programming language.
XEH playground is an interactive environment utilizing all language features, such as:
    - Live coding
    - Reverse Debugging
    - Snapshots
...
The second is data and the third is text representation of each byte.
...
Scroll to exact offset using "Go To" 
    Click View -> Go To ...
First goto text is evaluated using fresh XEH interpreted.
Then result is used as the new offset.
Please note that "Go To" operate in bits, not in bytes.

## REPL

How to evaluate the code as normal people do.
First switch to the REPL mode in the menu.
For example, put a numer 8 on top of the stack.
Type the number, click run.
...
Now we want multiply 8 by 32, but  lets make a missprint and see whats happen.
    32 *8
Status line display the error message: undefined word *8.
Ugly dashline point to the error location.
